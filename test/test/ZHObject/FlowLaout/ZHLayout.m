...
//    //宽度
//    //我们设置为静态的2列
//    CGFloat WIDTH = (ScreenWidth-self.sectionInset.left-self.sectionInset.right-self.minimumInteritemSpacing) / 2;
//
//    //定义数组保存每一列的高度
...
//        UICollectionViewLayoutAttributes *attr = [UICollectionViewLayoutAttributes layoutAttributesForCellWithIndexPath:index];
...
//            //将新的item高度加入到短的一列
//            colHeight[0] = colHeight[0] + height +self.minimumInteritemSpacing;
//            width = 0;
//        }else{
//            colHeight[1] = colHeight[1] + self.minimumInteritemSpacing;
//            width = 1;
//        }
//        //设置item位置
//        attr.frame = CGRectMake(self.sectionInset.left + (self.minimumInteritemSpacing + WIDTH) * width, colHeight[width] - height - self.minimumLineSpacing, WIDTH, height);
//        [_attributeAttay addObject:attr];
//    }
//    //设置itemSize来确保滑动范围的正确 这里是通过将所有的item高度平均化，计算出来的(以最高的列位标准)
//    if (colHeight[0] > colHeight[1]) {
//        self.itemSize = CGSizeMake(WIDTH, (colHeight[0] - self.sectionInset.top) * 2 / _itemCount - self.minimumInteritemSpacing);
//    }else{
//        self.itemSize = CGSizeMake(WIDTH, (colHeight[1] - self.sectionInset.top) * 2 / _itemCount - self.minimumInteritemSpacing);
//    }
//}
