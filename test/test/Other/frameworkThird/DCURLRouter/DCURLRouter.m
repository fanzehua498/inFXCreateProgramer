...
+ (void)pushURLString:(NSString *)urlString animated:(BOOL)animated reverseBlock:(void(^)(id))reverse{
...
//+ (NSString *)query:(NSDictionary *)query urlStr:(NSString *)urlString{
//    NSMutableString *mutStr = [NSMutableString stringWithString:@"?"];
//    for (NSString *key in query.allKeys) {
//        [mutStr appendString:[NSString stringWithFormat:@"%@=%@&", key, query[key]]];
//    }
//    NSString *url = [NSString stringWithFormat:@"%@%@", urlString, [mutStr substringToIndex:mutStr.length - 1]];
//    return url;
//}
...
+ (void)pushURLString:(NSString *)urlString query:(NSDictionary *)query animated:(BOOL)animated reverseBlock:(void (^)(id _Nonnull))reverse{
    UIViewController *viewController = [UIViewController initFromString:urlString withQuery:query fromConfig:[DCURLRouter sharedDCURLRouter].configDict];
    viewController.reserveValue = ^(id value){
        if (reverse) {
            reverse(value);
...
+ (void)presentURLString:(NSString *)urlString animated:(BOOL)animated withNavigationClass:(Class)classType completion:(void (^ __nullable)(void))completion reverseBlock:(void (^)(id _Nonnull))reverse{
    UIViewController *viewController = [UIViewController initFromString:urlString fromConfig:[DCURLRouter sharedDCURLRouter].configDict];
    viewController.reserveValue = ^(id value){
        if (reverse) {
            reverse(value);
...
+ (void)presentURLString:(NSString *)urlString animated:(BOOL)animated completion:(void (^)(void))completion reverseBlock:(void (^)(id _Nonnull))reverse{
    UIViewController *viewController = [UIViewController initFromString:urlString fromConfig:[DCURLRouter sharedDCURLRouter].configDict];
    viewController.reserveValue = ^(id value){
        if (reverse) {
            reverse(value);
...
+ (void)presentURLString:(NSString *)urlString query:(NSDictionary *)query animated:(BOOL)animated withNavigationClass:(Class)classType completion:(void (^)(void))completion reverseBlock:(void (^)(id _Nonnull))reverse{
...
#pragma mark 移除当前控制器的上一个控制器
+ (void)removeLastViewController{
    UIViewController *childVC = [[DCURLNavgation sharedDCURLNavgation] lastViewController];
...
- (UIViewController *)currentRootViewController{
    return [DCURLNavgation sharedDCURLNavgation].currentRootViewController;
}
