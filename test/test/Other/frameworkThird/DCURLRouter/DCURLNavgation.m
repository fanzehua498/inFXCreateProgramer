...
#pragma mark 返回当前控制器的前一个控制器
- (UIViewController *)lastViewController{
    NSInteger count = self.currentNavigationViewController.childViewControllers.count;
    if (count<2) return nil;
    return self.currentNavigationViewController.childViewControllers[count - 2];
}
...
- (UIViewController*)currentRootViewController{
    UITabBarController *tabBarVC = (UITabBarController *)self.applicationDelegate.window.rootViewController;
    UINavigationController *naviVC = tabBarVC.selectedViewController;
    return naviVC.childViewControllers.firstObject;
}
