...
- (void)viewDidLoad {
    ...
    NSLog(@"%@",[self dateConversion:@"2019-05-10"]);
}
...
-(void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [DCURLRouter pushURLString:@"ZHTest://citylist" query:@{@"city":@"param"} animated:YES];
...
- (NSString *)dateConversion:(NSString *) date {
    
    NSDateFormatter *old = [[NSDateFormatter alloc] init];
    old.dateFormat = @"YYYY-MM-dd";
    NSDate *gregorDate = [old dateFromString:date];
    
    NSCalendar *gregorian = [[NSCalendar alloc]
                             initWithCalendarIdentifier:NSCalendarIdentifierBuddhist];
    NSUInteger unitFlags = NSCalendarUnitDay | NSCalendarUnitMonth | NSCalendarUnitYear;
    NSDateComponents *components = [gregorian components:unitFlags fromDate:gregorDate];
    NSCalendar *buddhist = [[NSCalendar alloc]
                            initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSDate *buddhistDate = [buddhist dateFromComponents:components];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setLocale:[NSLocale localeWithLocaleIdentifier:@"th_TH"]];
    dateFormatter.dateFormat = @"dd MMM YYYY";
    NSString *currentDateStr = [dateFormatter stringFromDate:buddhistDate];
